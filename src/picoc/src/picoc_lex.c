...
#define IS_BASE_DIGIT(c, b)                                                                                            \
    (((c) >= '0' && (c) < '0' + (((b) < 10) ? (b) : 10)) || (((b) > 10) ? IS_HEX_ALPHA_DIGIT(c) : false))
        ...
        if(Parser->FileName == pc->StrEmpty && (pc->InteractiveHead == NULL || Token == TokenEOF)) {
            ...
            if(pc->InteractiveHead == NULL ||
               (unsigned char*)Parser->Pos ==
                ...
                if(Parser->Pos !=
                   &pc->InteractiveCurrentLine->Tokens[pc->InteractiveCurrentLine->NumBytes - TOKEN_DATA_OFFSET]) {
                    /* scan for the line */
...
enum LexToken LexRawPeekToken(struct ParseState* Parser) { return (enum LexToken) * (unsigned char*)Parser->Pos; }
    ...
    if(pc->InteractiveHead == NULL) {
        ...
        if(EndParser->Pos >= StartParser->Pos &&
           EndParser->Pos < &pc->InteractiveCurrentLine->Tokens[pc->InteractiveCurrentLine->NumBytes]) {
            ...
            MemSize =
                &pc->InteractiveCurrentLine->Tokens[pc->InteractiveCurrentLine->NumBytes - TOKEN_DATA_OFFSET] - Pos;
...
void LexInteractiveCompleted(Picoc* pc, struct ParseState* Parser) {
    while(pc->InteractiveHead != NULL && !(Parser->Pos >= &pc->InteractiveHead->Tokens[0] &&
...
void LexInteractiveStatementPrompt(Picoc* pc) { pc->LexUseStatementPrompt = true; }
