...
void StdlibCalloc(struct ParseState* Parser, struct Value* ReturnValue, struct Value** Param, int NumArgs) {
    ReturnValue->Val->Pointer = PICOC_FN_CALLOC(Param[0]->Val->Integer, Param[1]->Val->Integer);
}

void StdlibRealloc(struct ParseState* Parser, struct Value* ReturnValue, struct Value** Param, int NumArgs) {
    ReturnValue->Val->Pointer = PICOC_FN_REALLOC(Param[0]->Val->Pointer, Param[1]->Val->Integer);
}
...
struct LibraryFunction StdlibFunctions[] = {{StdlibAtof, "float atof(char *);"},
